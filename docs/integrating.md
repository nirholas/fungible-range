# Integrating FungibleRange

## From Solidity

```solidity
import {FungibleRangeHook} from "fungible-range/src/hooks/FungibleRangeHook.sol";
...
const hook = getHook("fungible-range");
const address = hookAddress("fungible-range", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "FungibleRange"
...
More at https://fungible-range.pages.dev.
