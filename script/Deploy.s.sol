...
/**
 * @title DeployFungibleRange
 * @notice Deploys FungibleRange deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployFungibleRange is Script {
    ...
    int24 public _baseHalfWidth;

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    int24 public _maxHalfWidth;

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    uint256 public _triggerBps;

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    uint256 public _widenBps;

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    uint64 public _calmSeconds;
    ...
    function run() external {
        ...
        bytes memory creationCode = type(FungibleRangeHook).creationCode;
        bytes memory constructorArgs = abi.encode(manager, _baseHalfWidth, _maxHalfWidth, _triggerBps, _widenBps, _calmSeconds, shareName, shareSymbol);
        ...
        console2.log("FungibleRange ", predicted);
        ...
        vm.startBroadcast();
        FungibleRangeHook hook = new FungibleRangeHook{salt: salt}(manager, _baseHalfWidth, _maxHalfWidth, _triggerBps, _widenBps, _calmSeconds, shareName, shareSymbol);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("FungibleRange")),
